-- AVM ve mağazalar için kullanıcı yorumları ve puanları tablosu
...
COMMENT ON COLUMN reviews.mall_id IS 'Yorum yapılan AVM''nin ID''si';
COMMENT ON COLUMN reviews.store_id IS 'Yorum yapılan mağazanın ID''si';
COMMENT ON COLUMN reviews.rating IS 'Puanlama (1-5)';
COMMENT ON COLUMN reviews.comment IS 'Yorum metni';
...
COMMENT ON COLUMN reviews.rejection_reason IS 'Reddedilme nedeni'; 